/**
 * deploy_lista_completa.js
 *
 * 1. Altera tabela presencas: adiciona coluna status (confirmado/ausente)
 * 2. Reescreve confirmar.js — INSERT status=confirmado
 * 3. Reescreve cancelar.js  — UPSERT status=ausente
 * 4. Reescreve lista.js     — formato completo com data, horario, 3 categorias
 */
...
var sql = [
  "ALTER TABLE presencas ADD COLUMN IF NOT EXISTS status ENUM('confirmado','ausente') NOT NULL DEFAULT 'confirmado';",
  "ALTER TABLE presencas DROP INDEX IF EXISTS presencas_partida_jogador;",
  "ALTER TABLE presencas ADD UNIQUE KEY IF NOT EXISTS presencas_partida_jogador (partida_id, jogador_id);"
].join('\n');

fs.writeFileSync('/tmp/lista_migration.sql', sql);
try {
  execSync('sudo mysql appfut < /tmp/lista_migration.sql');
  console.log('[OK] Migracao banco aplicada');
} catch(e) {
  console.log('[SKIP] Migracao banco: ' + e.message.split('\n')[0]);
}
...
var newConfirmar = [
  ...
  "",
  "  await delay();",
  "  await enviarListaCompleta(client, message.from, partida);",
  "}",
  "",
  "async function enviarListaCompleta(client, to, partida) {",
  "  try {",
  "    var texto = montarLista(await buscarDadosLista(partida), partida);",
  "    await client.sendText(to, texto);",
  "  } catch(e) { console.error('[Lista] Erro:', e); }",
  "}",
  "",
  "async function buscarDadosLista(partida) {",
  "  const db2 = require('../../database/connection');",
  "  const [confirmados] = await db2.execute(",
  ...
  "    [partida.id]",
  "  );",
  "  const [ausentes] = await db2.execute(",
  ...
  "    [partida.id]",
  "  );",
  "  const [duvida] = await db2.execute(",
  ...
  "  );",
  "  const [avulsos] = await db2.execute(",
...
var newCancelar = [
  ...
  "  await client.sendText(message.from, montarLista(dados, partida));",
  "}",
  "",
